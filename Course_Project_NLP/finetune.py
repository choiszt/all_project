...
def do_train():
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        save_dir = os.path.join(args.save_dir, "model_%d" % epoch)
...
def getparam(model): #统计模型大小
    num_params = sum(paddle.numel(param) for param in model.parameters())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", choices=["uie-senta-base", "uie-senta-medium", "uie-senta-mini", "uie-senta-micro", "uie-senta-nano"], default="uie-senta-micro", type=str, help="Select the pretrained model for few-shot learning.")
    parser.add_argument("--init_from_ckpt", default=None, type=str, help="The path of model parameters for initialization.")

    args = parser.parse_args()
    # yapf: enable
    args.save_dir="/mnt/ve_share/liushuai/PaddleNLP/applications/sentiment_analysis/unified_sentiment_extraction/liushuai_workdir/results/medium"
    args.num_epochs=5
    args.train_path="/mnt/ve_share/liushuai/PaddleNLP/applications/sentiment_analysis/unified_sentiment_extraction/Data/json/train.json"
    args.dev_path="/mnt/ve_share/liushuai/PaddleNLP/applications/sentiment_analysis/unified_sentiment_extraction/Data/json/dev.json"
    args.model="uie-senta-micro"
